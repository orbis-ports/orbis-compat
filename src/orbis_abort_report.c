...
/* ⚠ THE PATH IS NOT A PRODUCT NAME ANY MORE. This file was written in a frontend's tree and hard
 * coded /data/retroarch-abort.log; a copy in a game's tree hard coded /data/sonic3air-abort.log,
 * which is the whole reason the two copies existed. The default is generic, and a title that wants
 * its own name sets ORBIS_ABORT_LOG in /data/orbis-env.txt - read through orbis_env_get, so it
 * works on a console with no rebuild. */
#define ORBIS_ABORT_LOG_DEFAULT "/data/orbis-abort.log"
...
void orbis_report(const char *tag, const char *fmt, ...)
{
   ...
   {
      /* ⚠ orbis_env_get, NOT getenv: setenv() in one image is invisible to another here, so the
       * file on /data is the only channel. It is read at report time rather than cached, because
       * this path runs once per process death and a cache would be one more thing to be wrong. */
      const char *logpath = orbis_env_get("ORBIS_ABORT_LOG");
      if (logpath == NULL || *logpath == '\0')
         logpath = ORBIS_ABORT_LOG_DEFAULT;
